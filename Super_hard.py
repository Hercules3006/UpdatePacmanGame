...
class Bomb(StaticEntity):
    def __init__(self, xPos, yPos):
        super().__init__(xPos, yPos, 23)
        self.image = pygame.image.load('res/img/bom1.png')
    def draw(self):
...
def checkGhostCollisionFrighten():
    global score, gameOver, level, k_up, k_down, k_left, k_right, entities, ghosts, wall, blinkGhost, clydeGhost, inkyGhost, pacman, timerStart, pinkyGhost, bombs
    ...
    for i in bombs:
        if r.collidepoint((i.getXPos() + i.getSize() / 2, i.getYPos() + i.getSize() / 2)):
            if isinstance(i, Bomb):
                forcePlayMusic("bom.mp3")
                for _ in range(10):
                    image = pygame.image.load('res/img/bom2.png')
                    screen.blit(image, (i.xPos, i.yPos))
                i.setDead()
                bombs.remove(i)
                if pacman.lives > 1:
                    pacman.lives -= 1
    ...
    for i in range(len(level)):
        for j in range(len(level[i])):
            if level[i][j] == ".":
                ...
                entities.append(Bomb(j * size, i * size))
...
def main():
    ...
    global musicPlaying, blinkGhost, game_first_input, running, gameOver, bombs
    init()

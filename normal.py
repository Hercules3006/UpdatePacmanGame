...
def drawString(name, xPos, yPos,textColor=(255, 255, 255)):
    font = pygame.font.Font("res/font/font.ttf", 10)
    text = font.render(name, False, textColor)
...
def forcePlayMusic(music):
    ...
    musicPlaying = 1
###H
def TimeMode(remainTime):
    drawString(f"Time: {round(remainTime)}s",0,0,(255,255,0))
#####
...
def main():
    ...
    init()
    #H
    Mode=1 #Mode = 1 la mod choi tinh thoi gian
    start_time=0.0
    elapsed_time=0.0
    ######
    timerCount = 0
    ...
    while running:
        ...
        if win:
            ...
            if timerCount < (60 * 1):
                ...
                update()
            if(Mode==1):  #H
                input_time=120 #thoi gian choi, tinh bang giay
                elapsed_time = time.time() - start_time #H
                remain_time = input_time - elapsed_time #H
                TimeMode(remain_time)
                if(remain_time<=0): #H
                    start_time=0.0
                    elapsed_time=0.0
                    gameOver=True #H
